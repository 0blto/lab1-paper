...
### <a name="nuxt">Nuxt.js</a>

<p>Приложение на Nuxt.js и рендеримые на сервере страницы создаются по тому же принципу, только за основу взята структура, расширения и синтаксис Vue.js.</p>

<p>Для создания приложения используется команда:</p>
...
<p>Эта страница будет доступна по URL '/', используется та же структура, как в Next.js. Для создания динамических URL'ов название файла имеет вид _argument.vue, где argument это значение в ссылке. Метод asyncData в Nuxt.js используется для выполнения кода на сервере и предварительного заполнения данных перед рендерингом компонента на стороне клиента. Этот метод должен вернуть объект с данными.</p>

<p>Запуск осуществляется командой</p>
<pre>npm run dev</pre>

## <a name="где">Где использовать?</a>

<p>Как SSR, так и CSR имеют много преимуществ. CSR все еще остается актуальным решением, особенно если количество контента на сайте сравнительно небольшое. Тогда первая загрузка произойдет сравнительно быстро и дальнейшая интерактивность будет лучше. Если целевая аудитория - обладатели мощных компьютеров с постоянным высокоскоростным интернетом, то CSR так же остается актуальным решением.</p>

<p>SSR же дает меньшую интерактивность, изза частых запросов на сервер. Однако это даст большое преимущество, если среди целевой аудитории есть обладатели маломощных устройств или на которых не поддерживается JS, подразумевается доступность сайта с мобильного интернета, наличие большого количества данных и необходимость активного продвижения и рекламы сайта для набора клиентской базы. Примерами таких сайтов являются порталы, маркетплейсы и многие другие масштабные проекты.</p>
